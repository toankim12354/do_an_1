...
<body>
	...
	$sql = "SELECT * FROM san_pham LIMIT $so_san_pham_tren_trang OFFSET $so_san_pham_bo_qua";
			...
			<id id="content">
				...
				<div id="row1" >
					...
					<div id="row2">
								...
								<div class="right-row1-row1">
									<?php while ($sp = mysqli_fetch_assoc($result)	) { ?>
										<div class="item-right">
											...
											<h2><a style="text-decoration: none; color: #02FA31" href="#"><?php echo $sp['ten_sp']; ?></a></h2>

											<p style="color: #080808;">
												Giá bán:
												<span class="gia_do">    <?php echo number_format($sp['gia_sp'], 0, ',', '.'); ?> VNĐ
												</span>
											</p>
											...
											<?php 	} ?>
											<button><a  style="text-decoration: none;" href="chi_sp.php?ma_sp=<?php echo $sp['ma_sp'];	?>" >xem chi tiết    </a></button>
										</div>
						...
						<br>
						<div>
							<a href="?trang=<?= ($trang_hien_tai > 1) ? $trang_hien_tai - 1 : 1 ?> " style=" text-decoration: none;" ></a>
							<?php 	for ($i	= 1; $i <= $so_trang ; $i++) { ?>
								<a  style="display: inline-block; width: 20px;" href="?trang=<?= $i?>" style=" text-decoration: none;"><?= $i ?></a>
							<?php } ?>
							<a href="?trang=<?= ($trang_hien_tai < $so_trang) ? $trang_hien_tai + 1 : $so_trang ?>" style=" text-decoration: none;" 	>next</a>
						</div>
					</div>
				</div>
